...
#define WMBUS_PHY_CONFIG_DELAY              2
#define PHY_BUFF_MAX_LENGTH         290/*including crc*/
#define USER_DATA_MAX_LENGHT        255
...
#define N_MODE_BASE_FREQUENCY   169406250
...
#define N_AB_EF_MODE_MODULATION_SCHEME      GFSK_BT05
#define N_CD_MODE_MODULATION_SCHEME         GFSK_BT05
...
#define N_MODE_PHY_LENGTH_TYPE                 PKT_LENGTH_FIX
...
#define POSTABMLE_MAX_LEN               (uint8_t)0x08
...
#define DEFAULT_S1_POSTAMBLE_LEN                0x04
#define DEFAULT_S1m_PREAMBLE_LEN                  45
#define DEFAULT_S1m_POSTAMBLE_LEN               0x08
#define DEFAULT_S2_PREAMBLE_LEN                   45
#define DEFAULT_S2_PREAMBLE_LEN_L                344
#define DEFAULT_S2_POSTAMBLE_LEN                0x08
#define DEFAULT_T_PREAMBLE_LEN          45
#define DEFAULT_T_POSTAMBLE_LEN       0x04
#define DEFAULT_R2_PREAMBLE_LEN         96          
#define DEFAULT_R2_POSTAMBLE_LEN      0x08
#define DEFAULT_N_MODE_PREAMBLE_LENGTH        PKT_PREAMBLE_LENGTH_04BYTES  
#define DEFAULT_N_MODE_POSTAMBLE_LENGTH        0x00
...
/** @addtogroup WMBus_Phy_Exported_Types          WMBus Phy Exported Types
* @{
*/

/**
* @brief  WMBusMode_T for WMBus Phy enumeration
*/
typedef enum {
...
/**
* @brief  WMBusChannel_T for WMBus Phy enumeration
*/
typedef enum{
  ...
  CHANNEL_1a,/*!<Support for Channel 1a*/
...
/**
* @brief  WMBusDeviceType_T for WMBus Phy enumeration
*/
typedef enum {
...
/**
* @brief  FrameFormatType_t for WMBus Phy enumeration
*/
typedef enum
{
...
/**
* @brief  WMBusPhyState for WMBus Phy enumeration
*/
typedef enum {
...
/**
* @brief  WMBUS_PHY_ATTR for WMBus Phy enumeration
*/
typedef enum {
...
/**
* @brief  WMBus_PhyAttr_t  WMBus Phy exported structures
*/
typedef struct
{
  WMBusMode_T   DevMode;
...
/**
* @brief  PHY_Frame_t  WMBus Phy exported structures
*/
typedef struct
{
  uint8_t DataBuff[PHY_BUFF_MAX_LENGTH];/*!<Data buffer*/
  uint16_t length;/*!<Data Length*/
  uint16_t lenRemain;/*!<Remaining data length*/
}PHY_Frame_t;
...
*/
extern PHY_STATUS WMBus_PhyInit(WMBus_PhyAttr_t *pWMBusCurrPar);
...
*                2. uint32 *ParamVal: The pointer to the variable to set the attribute value.
* @retval PHY_STATUS: The status of execution
*/
extern PHY_STATUS WMBus_PhyGetAttr(WMBUS_PHY_ATTR attr, uint32_t *pParamVal);

/**
* @brief  This Routine will set the WMBus PHY attributes.
* @param  Input: 1. WMBUS_PHY_ATTR attr - The WMBus attribute.
*                2. uint32 ParamVal: The value of the attribute.
* @retval PHY_STATUS: The status of execution.
*/
extern PHY_STATUS WMBus_PhySetAttr(WMBUS_PHY_ATTR attr, uint32_t ParamVal);
...
*                       
*/
extern PHY_STATUS  WMBus_PhyTxDataHandleMode (uint8_t *pBuff, uint16_t length,\
...
extern PHY_STATUS  WMBus_PhyRxDataHandlerMode(uint32_t RxTimeoutDes, \
  void (*WMBus_LinkLayerParser)(uint8_t *pLinkRxFrameBuffer,\
...
*/
extern PHY_STATUS  WMBus_PhyGetState (WMBusPhyState *pPhyState);
...
*                       
*/
extern PHY_STATUS  WMBus_PhyReadConfig (WMBus_PhyAttr_t *pWMBusCurrPar);


void LoadConfRegs(uint8_t index);
int RadioCmp(SRadioInit *pr1, SRadioInit *pr2);
void ReInitRxCycle(void);
void SaveConfRegs(uint8_t index);
void StoreSpiritRegsBothDir(void);
void ResetActualStatus(void);
PHY_STATUS WMBus_PhyConfig(SpiritBool trx_mode);
uint8_t WMBus_PhyConfigFull(void);
PHY_STATUS  WMBus_PhyResetRxBuffer(void);
PHY_STATUS  WMBus_PhyResetTxBuffer(void);
void  WMBus_PhyRxPacketHandler(PHY_STATUS phyStatus);
int WMbusCmp(PktMbusInit *pw1, PktMbusInit *pw2);
